...
class Configure extends MethodForm
{
	...
	public function createForm(GDT_Form $form) : void
	{
		...
		$deps = array_map(function(GDO_Module $m) {
			return $m->getName();
		}, $deps);
		...
		$form->addField(GDT_Name::make('module_name')->writeable(false));
		$form->addField(GDT_String::make('module_path')->writeable(false)->initial($mod->filePath()));
		$c = GDT_Container::make('versions')->horizontal(false);
		$c->addField(GDT_Version::make('module_version')->writeable(false));
		...
		{
			$form->addField(GDT_Divider::make()->label('form_div_config_vars'));
