...
function printUsage(int $code = -1, ?string $cmd = null): void
{
    ...
    $commands = [
        ...
        'nginx' => "$exe %CMD% - To print an nginx server configuration.",
        'mysql' => "$exe %CMD% [<grant-host>] - To print SQL that creates the configured MySQL database and user.",
...
elseif ($command === 'nginx')
{
    $docs = Webserver::make()->nginx();
    echo $docs->render();
}
...
elseif ($command === 'mysql')
{
    ...
    if (GDO_DB_NAME === '')
    {
        fwrite(STDERR, "GDO_DB_NAME must not be empty.\n");
        die(1);
    }

    if (GDO_DB_USER === '')
    {
        fwrite(STDERR, "GDO_DB_USER must not be empty.\n");
        die(1);
    }

    # GDO_DB_HOST is the server to connect to, whereas a MySQL account host
    # describes where the application connects from. Localhost is the safe
    # default and can be overridden with the optional command argument.
    $grantHost = $argv[2] ?? 'localhost';
    ...
    $string = static function (string $value): string
    {
        return "'" . str_replace(
            ["\\", "\0", "\n", "\r", "\x1a", "'"],
            ["\\\\", "\\0", "\\n", "\\r", "\\Z", "''"],
    ...
    $database = $identifier(GDO_DB_NAME);
    $user = $string(GDO_DB_USER);
    $host = $string($grantHost);
    $password = $string(GDO_DB_PASS);

    echo "-- Generated by ./gdo_adm.sh mysql\n";
    printf(
        "-- Connect to MySQL at %s:%d as a privileged user and run this script.\n\n",
        GDO_DB_HOST,
        GDO_DB_PORT,
    );
    ...
    echo "CREATE USER IF NOT EXISTS {$user}@{$host} IDENTIFIED BY {$password};\n";
    echo "ALTER USER {$user}@{$host} IDENTIFIED BY {$password};\n";
