...
find ./ -maxdepth 1 -type d -iname '.git' -print0 | xargs $XARGS_OPTIONS bash -c "cd \"{}\"/../ && OUT=\"\$(echo \"{}\" | cut -f 3 -d '/')\" && echo -e \"-----------------------------\nupdating repo [ \\\"\$(pwd)\\\" ]:\" >> temp/git_pull_\$OUT && LANG=en_GB LC_ALL=en_GB git pull &>> temp/git_pull_\$OUT && git submodule foreach --recursive 'if branch=\$(git symbolic-ref --quiet --short HEAD); then git pull --ff-only; else branch=\$(git symbolic-ref --quiet --short refs/remotes/origin/HEAD) && branch=\${branch#origin/} && git checkout \"\$branch\" && git pull --ff-only; fi' >> temp/git_pull_\$OUT && git submodule update --recursive --remote &>> temp/git_pull_\$OUT ; cat temp/git_pull_\$OUT && rm temp/git_pull_\$OUT "
